...
class ExantrixCommandeModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function postProcess()
    {
        ...
        $signature = isset($_SERVER['HTTP_X_EXANTRIX_SIGNATURE']) ? $_SERVER['HTTP_X_EXANTRIX_SIGNATURE'] : '';
        $attendue = 'sha256=' . hash_hmac('sha256', $corps, (string) Configuration::get('EXANTRIX_JETON'));
        ...
        $c = $donnees['commande'];
        Db::getInstance()->execute('INSERT IGNORE INTO `' . _DB_PREFIX_ . 'exantrix_commande` (id_commande_exantrix, donnees, date_add) VALUES ('
        ...
        $texte = 'Commande Exantrix #' . (int) $c['id'] . "\n\n" . $lignes . "\nLivraison : " . ($liv['prenom'] ?? '') . ' ' . ($liv['nom'] ?? '') . ', '
            . ($liv['adresse'] ?? '') . ', ' . ($liv['code_postal'] ?? '') . ' ' . ($liv['ville'] ?? '') . ' - ' . ($liv['telephone'] ?? '')
            . "\n\nDetail et expedition : https://exantrix.com/a2/vendeur";
        @mail(Configuration::get('PS_SHOP_EMAIL'), 'Commande Exantrix #' . (int) $c['id'], $texte);
